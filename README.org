	...
	if (FALSE){
	  ...
	  bind_rows( 
	   ...
	   ,dat %>%
		...
		labs(
		 ...
		 ,x = ''
		 ,y = 'número de respostas'
		 ,caption = 'Obs.: "Boa, porque familiar me ajuda" reclassificada como "Ruim"'
		 ,fill = 'Serviço'
		) +
		theme( legend.position = 'top') +
		theme_bw()
	  ...
	  bind_rows(
	   ...
	   ,dat %>%
		...
		facet_wrap( ~ escolaridade, scales = 'free_y') + 
		theme( legend.position = 'top') +
		theme_bw()
	  ...
	  bind_rows(
	   ...
	   ,dat %>%
		...
		facet_wrap( ~ uso_rede, scales = 'free_y') + 
		theme( legend.position = 'top') +
		theme_bw()
