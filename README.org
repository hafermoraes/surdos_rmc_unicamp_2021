   ...
   evol_plot <- evol %>%
	 ...
	 labs(
	  ...
	  ,caption = paste(
		...
		,format( max(dat$V001), '%d.%m.%Y %H:%M:%S' )
	   )
	 )
...
** Perfil dos participantes
   ...
   dat %>%
	 ...
	 mutate( p = round( n/sum(n), digits = 3) ) %>% 
	 {
	   bind_rows(
		 .
		 ,data.frame( V010 = 'RESPOSTAS', n=nrow(dat), p=1 )
	   )
	 }
   #+END_SRC
   ...
   - idade média de src_R[:session :results replace ]{ round( v004,1) } {{{results(=38.5=)}}} anos
   - src_R[:session :results replace ]{ round( v005[1,3]*100,1) } {{{results(=54.3=)}}} % do sexo src_R[:session :results replace ]{ v005[1,1] } {{{results(=Feminino=)}}}
   - src_R[:session :results replace ]{ 100*sum( v006[ c(4,5), 'p'] ) } {{{results(=64.2=)}}} % com ensino superior ou pós-graduação
   - src_R[:session :results replace ]{ 100*sum( v015[ c(1,2), 'p'] ) } {{{results(=74.1=)}}} % avaliam seu estado de saúde como bom ou muito bom
