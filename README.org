...
** Perfil dos participantes
   ...
   Até o momento src_R[:session :results replace ]{ nrow(dat)} {{{results(=235=)}}}  pessoas participaram da pesquisa, provenientes das seguintes cidades
   ...
   | HORTOLANDIA            |   1 | 0.004 |
   | HORTOLANDIA E CAMPINAS |   1 | 0.004 |
   | HOTOLANDIA             |   1 | 0.004 |
   ...
   Dos quais até o momento src_R[:session :results replace ]{ nrow(dat %>% filter( !is.na(VALIDA)))} {{{results(=197=)}}}  participantes pertencem ao público alvo da pesquisa, provenientes das seguintes cidades
   ...
   - idade média de src_R[:session :results replace ]{ round( v004,1) } {{{results(=49.5=)}}} anos
   ...
   - src_R[:session :results replace ]{ 100*sum( v006[ c(4,5), 'p'] ) } {{{results(=43.7=)}}} % com ensino superior ou pós-graduação
   - src_R[:session :results replace ]{ 100*sum( v015[ c(1,2), 'p'] ) } {{{results(=74.6=)}}} % avaliam seu estado de saúde como bom ou muito bom
