...
** Ambiente de computação
   ...
   other attached packages:
	[1] factoextra_1.0.7 FactoMineR_2.3   scales_1.1.1     gtsummary_1.4.2 
	[5] haven_2.4.0      xtable_1.8-4     stringr_1.4.0    tidyr_1.1.2     
	[9] dplyr_1.0.3      readr_1.4.0      ggplot2_3.3.3   

   loaded via a namespace (and not attached):
	[1] Rcpp_1.0.6           pillar_1.6.0         compiler_3.6.3      
	[4] forcats_0.5.1        tools_3.6.3          digest_0.6.27       
	[7] lattice_0.20-41      lifecycle_1.0.0      tibble_3.1.1        
   [10] gtable_0.3.0         pkgconfig_2.0.3      rlang_0.4.11        
   [13] Matrix_1.2-18        rstudioapi_0.13      DBI_1.1.1.9000      
   [16] cli_2.4.0            ggrepel_0.8.2        xfun_0.26           
   [19] cluster_2.1.0        knitr_1.32           withr_2.4.2         
   [22] generics_0.1.0       vctrs_0.3.8          hms_1.0.0           
   [25] flashClust_1.01-2    scatterplot3d_0.3-41 grid_3.6.3          
   [28] tidyselect_1.1.0     glue_1.4.2           R6_2.5.0            
   [31] fansi_0.4.2          survival_3.2-3       purrr_0.3.4         
   [34] blob_1.2.1           magrittr_2.0.1       ps_1.6.0            
   [37] MASS_7.3-51.6        leaps_3.1            splines_3.6.3       
   [40] htmltools_0.5.1.1    ellipsis_0.3.2       broom.helpers_1.4.0 
   [43] assertthat_0.2.1     gt_0.3.1             colorspace_2.0-0    
   ...
   dat <- raw %>%
	 ...
	 transmute(
	  ...
	  ,V002
	   ...
	   ## correção pontual não será mais feita...
	   ## ,V003 = case_when(
	   ##    V001 == '06/10/2021 22:23:42' & is.na(V003)     ~ 'A - Sim' # TCLE e questionário respondidos, publico alvo vazio
	   ##   ,V001 == '06/10/2021 20:49:53' & is.na(V003)     ~ 'A - Sim' # TCLE e questionário respondidos, publico alvo vazio
	   ##   ,TRUE ~ as.character(V003)
	   ##  )
	  ,V003 = factor(
...
** Perfil dos participantes
   ...
   Destes, src_R[:session :results replace ]{ nrow(dat %>% filter( VALIDA == TRUE ))} {{{results(=188=)}}}  participantes pertencem ao público alvo da pesquisa, provenientes das seguintes cidades
   ...
   Embora tenham consentido ao TCLE e confirmado residir na RMC, src_R[:session :results replace ]{ nrow(v010) } {{{results(=1=)}}} respondentes não informaram o município no qual moram; destes, src_R[:session :results replace ]{ v010 %>% filter( !is.na(V012) ) %>% nrow } {{{results(=0=)}}} informaram o CEP, do qual é possível confirmar que de fato residem na RMC. Consideramos todos eles como respostas válidas à pesquisa.

   Perfil dos participantes:

   - idade média de src_R[:session :results replace ]{ round( v004,1) } {{{results(=38.9=)}}} anos
   - src_R[:session :results replace ]{ round( v005[1,3]*100,1) } {{{results(=56.4=)}}} % do sexo src_R[:session :results replace ]{ v005[1,1] } {{{results(=Feminino=)}}}
   - src_R[:session :results replace ]{ 100*sum( v006[ c(4,5), 'p'] ) } {{{results(=38.8=)}}} % com ensino superior ou pós-graduação
   - src_R[:session :results replace ]{ 100*sum( v015[ c(1,2), 'p'] ) } {{{results(=74.4=)}}} % avaliam seu estado de saúde como bom ou muito bom
...
*** Quanto ao sexo
	...
	| Feminino                   | 106 | 0.564 |
	| Masculino                  |  82 | 0.436 |
	| Total de respostas válidas | 188 |     1 |
...
*** Quanto à escolaridade
	...
	Embora src_R[:session :results replace]{  round(100*sum( v006[1:3,3] ),1)  } {{{results(=61.1=)}}} % dos respondentes tenham escolaridade até o ensino médio, o que confirma a hipótese prévia da pesquisa, ressalta-se também o alto número de surdos com alta escolaridade (src_R[:session :results replace]{ round( 100*sum( v006[4:5,3] ), 1)} {{{results(=38.8=)}}} %).

	Em nosso ponto de vista isto se constitui em um viés da amostra, decorrente também da elevada idade média dos respondentes (src_R[:session :results replace ]{ round( v004,1) } {{{results(=38.9=)}}} anos), sugerindo que a amostra não conseguiu 'furar a bolha' dos surdos próximos à universidade.
	...
	| V006                                                |   n |     f |
	|-----------------------------------------------------+-----+-------|
	| Nunca estudei                                       |   4 | 0.021 |
	| Fundamental (ou 1º grau)                            |  23 | 0.122 |
	| Médio (ou 2º grau)                                  |  88 | 0.468 |
	| Superior (ou 3º grau)                               |  56 | 0.298 |
	| Pós-graduação (especialização, mestrado, doutorado) |  17 |  0.09 |
	| Total de respostas válidas                          | 188 |     1 |
...
*** Quanto à religião
	...
	| Evangélica                 |  79 |  0.42 |
	| Católica                   |  56 | 0.298 |
	| Sem religião               |  29 | 0.154 |
	| Espírita                   |   7 | 0.037 |
	| Outra                      |   6 | 0.032 |
	| Adventista                 |   3 | 0.016 |
	| Umbanda / Candomblé        |   3 | 0.016 |
	| Não informada              |   2 | 0.011 |
	| Testemunha de Jeová        |   2 | 0.011 |
	| Metodista                  |   1 | 0.005 |
	| Total de respostas válidas | 188 |     1 |
...
*** Quanto à situação de emprego
	...
	| V007                                     |   n |     f |
	|------------------------------------------+-----+-------|
	| Sim, trabalho                            | 122 | 0.649 |
	| Não, estou desempregado                  |  43 | 0.229 |
	| Não, porque sou aposentado               |  13 | 0.069 |
	| Não trabalho e recebo benefício BPC/INSS |  10 | 0.053 |
	| Total de respostas válidas               | 188 |     1 |
...
**** segmentada por nível de escolaridade
  ...
  :END:  
	...
	| Fundamental (ou 1º grau)                            |            4 |         7 |            36.36 |
	| Médio (ou 2º grau)                                  |           22 |        63 |            25.88 |
	| Superior (ou 3º grau)                               |           13 |        39 |               25 |
	| Pós-graduação (especialização, mestrado, doutorado) |            3 |        13 |            18.75 |
...
**** segmentada por sexo
  ...
  :END:  
	...
	|-----------+--------------+-----------+------------------|
	| Feminino  |           32 |        63 |            33.68 |
	| Masculino |           11 |        59 |            15.71 |
...
*** Quanto à estrutura pública adequada de comunicação paciente-médico
	...
	| V019                                                |   n |     f |
	|-----------------------------------------------------+-----+-------|
	| Ruim, não sabem LIBRAS e sou obrigado a oralizar    |  34 | 0.243 |
	| Ruim, não sabem LIBRAS e preciso escrever português |  21 |  0.15 |
	| Ruim, não consigo comunicação                       |  39 | 0.279 |
	| Boa, tem intérprete que me ajuda                    |  14 |   0.1 |
	| Boa, porque familiar me ajuda                       |  30 | 0.214 |
	| Boa, o profissional de saúde sabe LIBRAS            |   2 | 0.014 |
	| Total de respostas válidas                          | 140 |     1 |
	...
	|--------------------------------------------+-----+-------|
	| Sim                                        |  22 | 0.157 |
	| Não, porque eu esqueço de pedir            |  22 | 0.157 |
	| Não, porque não tem intérprete livre nunca |  38 | 0.271 |
	| nil                                        |  58 | 0.414 |
	| Total de respostas válidas                 | 140 |     1 |
	...
	| V025                                               |  n |     f |
	|----------------------------------------------------+----+-------|
	| Ruim, não sabe LIBRAS e sou obrigado a oralizar    | 14 | 0.326 |
	| Ruim, não sabe LIBRAS e preciso escrever português |  5 | 0.116 |
	| Ruim, não consigo comunicação                      |  9 | 0.209 |
	| Boa, tem intérprete que me ajuda                   |  2 | 0.047 |
	| Boa, porque familiar me ajuda                      | 10 | 0.233 |
	| Boa, o profissional de saúde sabe LIBRAS           |  2 | 0.047 |
	| nil                                                |  1 | 0.023 |
	| Total de respostas válidas                         | 43 |     1 |
	...
	| V028                                               | n |     f |
	|----------------------------------------------------+---+-------|
	| Ruim, não sabe LIBRAS e sou obrigado a oralizar    | 3 | 0.429 |
	| Ruim, não sabe LIBRAS e preciso escrever português | 1 | 0.143 |
	| Boa, porque familiar me ajuda                      | 2 | 0.286 |
	| Boa, o profissional de saúde sabe LIBRAS           | 1 | 0.143 |
	| Total de respostas válidas                         | 7 |     1 |
...
*** Quanto à estrutura privada adequada de comunicação paciente-médico
	...
	| V022                                                |   n |     f |
	|-----------------------------------------------------+-----+-------|
	| Ruim, não sabem LIBRAS e sou obrigado a oralizar    |  35 | 0.232 |
	| Ruim, não sabem LIBRAS e preciso escrever português |  23 | 0.152 |
	| Ruim, não consigo comunicação                       |  48 | 0.318 |
	| Boa, tem intérprete que me ajuda                    |  14 | 0.093 |
	| Boa, porque familiar me ajuda                       |  28 | 0.185 |
	| Boa, o profissional de saúde sabe LIBRAS            |   2 | 0.013 |
	| nil                                                 |   1 | 0.007 |
	| Total de respostas válidas                          | 151 |     1 |
...
** Comparação do serviço público versus serviço privado
	...
	tbl_subset <- tbl %>% count( estrutura, servico ) %>% rename( xlabel = 1) %>% na.omit()
	cap <- tbl_subset %>% group_by( servico ) %>% summarise( n = sum(n) ) %>% as.data.frame()
	plot <- tbl_subset %>%
	  ggplot( aes(x = xlabel, y = n, group = servico, fill = servico) ) +
	  geom_bar(stat = 'identity', position = position_dodge()) +
	  ...
	  theme_bw() + 
	  theme( legend.position = 'top')
	ggsave( plot, filename = './img/estrutura_agrp.png', width=5, height=4)

	## Facets (estrutura/ Escolaridade)
	tbl_subset <- tbl %>% count( estrutura, servico, escolaridade ) %>% rename( xlabel = 1) %>% na.omit()
	cap <- tbl_subset %>% group_by( servico ) %>% summarise( n = sum(n) ) %>% as.data.frame()
	plot <- tbl_subset %>%
	  ggplot( aes(x = xlabel, y = n, group = servico, fill = servico) ) +
	  geom_bar(stat = 'identity', position = position_dodge()) +
	  ...
	  theme_bw() + 
	  theme( legend.position = 'top')
	ggsave( plot, filename = './img/estrutura_escolaridade_agrp.png', width=11, height=5)

	## Facets (estrutura/ Cor-Raça)
	tbl_subset <- tbl %>% count( estrutura, servico, cor ) %>% rename( xlabel = 1) %>% na.omit()
	cap <- tbl_subset %>% group_by( servico ) %>% summarise( n = sum(n) ) %>% as.data.frame()
	plot <- tbl_subset %>%
	  ggplot( aes(x = xlabel, y = n, group = servico, fill = servico) ) +
	  geom_bar(stat = 'identity', position = position_dodge()) +
	  ...
	  theme_bw() + 
	  theme( legend.position = 'top')
	ggsave( plot, filename = './img/estrutura_cor_agrp.png', width=8, height=5)

	## Facets (estrutura/ sexo)
	tbl_subset <- tbl %>% count( estrutura, servico, sexo ) %>% rename( xlabel = 1) %>% na.omit()
	cap <- tbl_subset %>% group_by( servico ) %>% summarise( n = sum(n) ) %>% as.data.frame()
	plot <- tbl_subset %>%
	  ggplot( aes(x = xlabel, y = n, group = servico, fill = servico) ) +
	  geom_bar(stat = 'identity', position = position_dodge()) +
	  ...
	  theme_bw() + 
	  theme( legend.position = 'top')
	ggsave( plot, filename = './img/estrutura_sexo_agrp.png', width=8, height=5)
	...
	## Facets (estrutura/ uso-rede)
	tbl_subset <- tbl %>% count( estrutura, servico, uso_rede ) %>% rename( xlabel = 1) %>% na.omit()
	cap <- tbl_subset %>% group_by( servico ) %>% summarise( n = sum(n) ) %>% as.data.frame()
	plot <- tbl_subset %>%
	  ggplot( aes(x = xlabel, y = n, group = servico, fill = servico) ) +
	  geom_bar(stat = 'identity', position = position_dodge()) +
	  ...
	  theme_bw() + 
	  theme( legend.position = 'top') 
	ggsave( plot, filename = './img/estrutura_rede_agrp.png', width=8, height=5)
...
*** Quanto à estrutura de atendimento ao surdo usuário de Libras
  :PROPERTIES:
  :CUSTOM_ID: sec:comp_estrut
  :END:  

**** Avaliação geral da estrutura 

	 #+CAPTION: Avaliação geral da estrutura de atendimento ao surdo usuário de Libras
	 #+ATTR_ORG: :width 600
	 [[./img/estrutura_agrp.png]]

	
**** Segmentada por sexo

	 #+CAPTION: Avaliação da estrutura de atendimento ao usuário de Libras segmentada por sexo 
	 #+ATTR_ORG: :width 600
	 [[./img/estrutura_sexo_agrp.png]]
 

**** Segmentada por escolaridade

	 #+CAPTION: Avaliação da estrutura de atendimento ao usuário de Libras segmentada por escolaridade
	 #+ATTR_ORG: :width 600
	 [[./img/estrutura_escolaridade_agrp.png]]
	 
**** Segmentada por cor/raça

	 #+CAPTION: Avaliação da estrutura de atendimento ao usuário de Libras segmentada por cor/raça 
	 #+ATTR_ORG: :width 600
	 [[./img/estrutura_cor_agrp.png]]

**** Segmentada por uso de rede

	 #+CAPTION: Avaliação da estrutura de atendimento ao usuário de Libras segmentada por uso de rede
	 #+ATTR_ORG: :width 600
	 [[./img/estrutura_rede_agrp.png]]
...
*** Escolaridade vs Estado de Saúde
	...
	p-valor do teste qui-quadrado para independência entre as variáveis: src_R[:session :results replace ]{ round( ca.pvalue, 6) } {{{results(=0.555703=)}}}.
...
*** Religião vs Estado de Saúde
	...
	p-valor do teste qui-quadrado para independência entre as variáveis: src_R[:session :results replace ]{ round( ca.pvalue, 6) } {{{results(=0.133178=)}}}.
...
*** Trabalho vs Estado de Saúde
	...
	p-valor do teste qui-quadrado para independência entre as variáveis: src_R[:session :results replace ]{ round( ca.pvalue, 6) } {{{results(=0.079579=)}}}.
...
*** Cor/Raça vs Estado de Saúde
	...
	p-valor do teste qui-quadrado para independência entre as variáveis: src_R[:session :results replace ]{ round( ca.pvalue, 6) } {{{results(=0.687884=)}}}.
...
*** Cidade vs Estado de Saúde
	...
	p-valor do teste qui-quadrado para independência entre as variáveis: src_R[:session :results replace ]{ round( ca.pvalue, 6) } {{{results(=0.946396=)}}}.
...
*** Diabetes vs Estado de Saúde
	...
	p-valor do teste qui-quadrado para independência entre as variáveis: src_R[:session :results replace ]{ round( ca.pvalue, 6) } {{{results(=0.086958=)}}}.
...
*** Hipertensão vs Estado de Saúde
	...
	p-valor do teste qui-quadrado para independência entre as variáveis: src_R[:session :results replace ]{ round( ca.pvalue, 6) } {{{results(=0.003653=)}}}.
...
*** Tabagismo vs Estado de Saúde
	...
	p-valor do teste qui-quadrado para independência entre as variáveis: src_R[:session :results replace ]{ round( ca.pvalue, 6) } {{{results(=0.838445=)}}}.
...
*** Faixa etária vs Estado de Saúde
	...
	p-valor do teste qui-quadrado para independência entre as variáveis: src_R[:session :results replace ]{ round( ca.pvalue, 6) } {{{results(=0.602179=)}}}.
...
* Conclusões
 ...
 6. Quando analisamos a estrutura do atendimento ao paciente surdo, precisamos reclassificar uma das respostas como ruim, a dizer: "Boa, porque familiar me ajuda". Embora a comunicação seja possível nessa situação, o simples fato de um familiar precisar acompanhar o surdo em sua consulta demonstra que a estrutura de atendimento a ele é ruim (vide seção [[#sec:comp_estrut]] e suas subseções).
