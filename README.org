...
** Ambiente de computação
   ...
   [16] cli_2.4.0           commonmark_1.7      xfun_0.26          
   [19] knitr_1.32          withr_2.4.2         rappdirs_0.3.1     
   [22] sass_0.4.0          fs_1.5.0            generics_0.1.0     
   [25] vctrs_0.3.8         hms_1.0.0           grid_3.6.3         
   [28] tidyselect_1.1.0    glue_1.4.2          R6_2.5.0           
   [31] fansi_0.4.2         survival_3.2-3      farver_2.1.0       
   [34] purrr_0.3.4         blob_1.2.1          magrittr_2.0.1     
   [37] backports_1.2.1     ps_1.6.0            splines_3.6.3      
   [40] htmltools_0.5.1.1   scales_1.1.1        ellipsis_0.3.2     
   [43] broom.helpers_1.4.0 assertthat_0.2.1    gt_0.3.1           
   [46] colorspace_2.0-0    labeling_0.4.2      utf8_1.2.1         
   [49] stringi_1.5.3       munsell_0.5.0       broom_0.7.9        
   [52] crayon_1.4.1
   #+end_example
...
*** Quanto ao sexo
	...
	## sexo (V005)
	dat %>%
	  filter( V001 <= '2021-10-15', VALIDA == TRUE ) %>%
	  ...
	  bind_rows(
	   ...
	   ,data.frame(
		 ...
		 ,n=nrow(dat %>% filter( V001 <= '2021-10-15', VALIDA == TRUE ))
		 ,f=1
		)
	  )
	...
	| Feminino                   | 125 | 0.551 |
	| Masculino                  | 102 | 0.449 |
	| Total de respostas válidas | 227 |     1 |
...
*** Quanto à escolaridade
	...
	| Fundamental (ou 1º grau)                            |  26 | 0.115 |
	| Médio (ou 2º grau)                                  | 103 | 0.454 |
	| Superior (ou 3º grau)                               |  70 | 0.308 |
	| Pós-graduação (especialização, mestrado, doutorado) |  22 | 0.097 |
	| nil                                                 |   1 | 0.004 |
	| Total de respostas válidas                          | 227 |     1 |
...
*** Quanto à situação de emprego
	...
	| V007                                     |   n |     f |
	|------------------------------------------+-----+-------|
	| Sim, trabalho                            | 146 | 0.643 |
	| Não, estou desempregado                  |  52 | 0.229 |
	| Não, porque sou aposentado               |  17 | 0.075 |
	| Não trabalho e recebo benefício BPC/INSS |  12 | 0.053 |
	| Total de respostas válidas               | 227 |     1 |
...
*** Quanto à estrutura pública adequada de comunicação paciente-médico
	...
	dat %>%
	  filter( V001 <= '2021-10-15', VALIDA == TRUE, V018 == 'Sim' ) %>%
	  count( V019 ) %>%
	  mutate( f = round( n/sum(n), digits = 3) ) %>% 
	  bind_rows(
		.
	   ,data.frame(
		 ...
		 ,n=nrow(dat %>% filter( V001 <= '2021-10-15', VALIDA == TRUE, V018 == 'Sim' ))
		 ,f=1
		)
	  )
	#+END_SRC
	...
	| V019                                                |   n |     f |
	|-----------------------------------------------------+-----+-------|
	| Ruim, não sabem LIBRAS e sou obrigado a oralizar    |  38 | 0.224 |
	| Ruim, não sabem LIBRAS e preciso escrever português |  26 | 0.153 |
	| Ruim, não consigo comunicação                       |  42 | 0.247 |
	| Boa, tem intérprete que me ajuda                    |  19 | 0.112 |
	...
	dat %>%
	  filter( V001 <= '2021-10-15', VALIDA == TRUE, V023 == 'Sim', V024 == 'Sim' ) %>%
	  count( V025 ) %>%
	  mutate( f = round( n/sum(n), digits = 3) ) %>% 
	  bind_rows(
		.
	   ,data.frame(
		 ...
		 ,n=nrow(dat %>% filter( V001 <= '2021-10-15', VALIDA == TRUE, V023 == 'Sim', V024 == 'Sim' ))
		 ,f=1
		)
	  )
	#+END_SRC
	...
	dat %>%
	  filter( V001 <= '2021-10-15', VALIDA == TRUE, V026 == 'Sim', V027 == 'Sim' ) %>%
	  count( V028 ) %>%
	  mutate( f = round( n/sum(n), digits = 3) ) %>% 
	  bind_rows(
		.
	   ,data.frame(
		 ...
		 ,n=nrow(dat %>% filter( V001 <= '2021-10-15', VALIDA == TRUE, V026 == 'Sim', V027 == 'Sim' ))
		 ,f=1
		)
	  )
	#+END_SRC
...
*** Quanto à estrutura privada adequada de comunicação paciente-médico
	...
	dat %>%
	  filter( V001 <= '2021-10-15', VALIDA == TRUE, V021 == 'Sim' ) %>%
	  count( V022 ) %>%
	  mutate( f = round( n/sum(n), digits = 3) ) %>% 
	  bind_rows(
		.
	   ,data.frame(
		 ...
		 ,n=nrow(dat %>% filter( V001 <= '2021-10-15', VALIDA == TRUE, V021 == 'Sim' ))
		 ,f=1
		)
	  )
	#+END_SRC
