   ...
   dat <- raw %>%
	 transmute(
	  ...
	  ,VALIDA = case_when(
		 ...
		 str_sub( V002, end = 3) == 'Sim' & V003 == 'Sim' & V004_idade >= 18 & ( V010_rmc == TRUE | is.na(V010) ) ~ TRUE
		...
		,str_sub( V002, end = 3) != 'Sim' & V003 != 'Sim' & V004_idade >= 18 & ( V010_rmc == TRUE               ) ~ TRUE
	   )
	 )
...
** Perfil dos participantes
   ...
   - idade média de src_R[:session :results replace ]{ round( v004,1) } {{{results(=38.4=)}}} anos
   - src_R[:session :results replace ]{ round( v005[1,3]*100,1) } {{{results(=53.7=)}}} % do sexo src_R[:session :results replace ]{ v005[1,1] } {{{results(=Feminino=)}}}
   - src_R[:session :results replace ]{ 100*sum( v006[ c(4,5), 'p'] ) } {{{results(=64.7=)}}} % com ensino superior ou pós-graduação
...
* Matriz de distâncias
  ...
  - =TP_UNIDADE=, tipo de unidade, a ser relacionado com a tabela =tbTipoUnidade202107.csv=
