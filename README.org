   ...
   dat <- raw %>% 
	 ...
	 transmute(
	  ...
	  ,V002
	   ...
	   ## correção pontual em 21.10.2021
	  ,V003 = case_when(
	  ...
	  ,V004 = case_when(
		...
		,V001 == '22/09/2021 16:55:45' & is.na(V004)          ~ '01/01/2025' # excluir da pesquisa, por isso a data futura
...
** Perfil dos participantes
   ...
   Destes, src_R[:session :results replace ]{ nrow(dat %>% filter( VALIDA == TRUE ))} {{{results(=225=)}}}  participantes pertencem ao público alvo da pesquisa, provenientes das seguintes cidades
   ...
   Embora tenham consentido ao TCLE e confirmado residir na RMC, src_R[:session :results replace ]{ nrow(v010) } {{{results(=2=)}}} respondentes não informaram o município no qual moram; destes, src_R[:session :results replace ]{ v010 %>% filter( !is.na(V012) ) %>% nrow } {{{results(=0=)}}} informaram o CEP, do qual é possível confirmar que de fato residem na RMC. Consideramos todos eles como respostas válidas à pesquisa.

   Perfil dos participantes:

   - idade média de src_R[:session :results replace ]{ round( v004,1) } {{{results(=38.9=)}}} anos
   - src_R[:session :results replace ]{ round( v005[1,3]*100,1) } {{{results(=54.7=)}}} % do sexo src_R[:session :results replace ]{ v005[1,1] } {{{results(=Feminino=)}}}
   - src_R[:session :results replace ]{ 100*sum( v006[ c(4,5), 'p'] ) } {{{results(=39.6=)}}} % com ensino superior ou pós-graduação
   - src_R[:session :results replace ]{ 100*sum( v015[ c(1,2), 'p'] ) } {{{results(=75.1=)}}} % avaliam seu estado de saúde como bom ou muito bom
...
*** Quanto ao sexo
	...
	| Masculino                  | 100 | 0.444 |
	| nil                        |   2 | 0.009 |
	| Total de respostas válidas | 225 |     1 |
...
*** Quanto à escolaridade
	...
	| V006                                                |   n |     f |
	|-----------------------------------------------------+-----+-------|
	| Nunca estudei                                       |   4 | 0.018 |
	| Fundamental (ou 1º grau)                            |  26 | 0.116 |
	| Médio (ou 2º grau)                                  | 104 | 0.462 |
	| Superior (ou 3º grau)                               |  69 | 0.307 |
	| Pós-graduação (especialização, mestrado, doutorado) |  20 | 0.089 |
	| nil                                                 |   2 | 0.009 |
	| Total de respostas válidas                          | 225 |     1 |
...
*** Quanto à situação de emprego
	...
	| V007                                     |   n |     f |
	|------------------------------------------+-----+-------|
	| Sim, trabalho                            | 148 | 0.658 |
	| Não, estou desempregado                  |  50 | 0.222 |
	| Não, porque sou aposentado               |  14 | 0.062 |
	| Não trabalho e recebo benefício BPC/INSS |  13 | 0.058 |
	| Total de respostas válidas               | 225 |     1 |
...
**** segmentada por nível de escolaridade
  ...
  :END:  
	...
	| Médio (ou 2º grau)                                  |           27 |        73 |               27 |
	| Superior (ou 3º grau)                               |           15 |        49 |            23.44 |
	| Pós-graduação (especialização, mestrado, doutorado) |            3 |        16 |            15.79 |
...
**** segmentada por sexo
  ...
  :END:  
	...
	|-----------+--------------+-----------+------------------|
	| Feminino  |           35 |        74 |            32.11 |
	| Masculino |           15 |        72 |            17.24 |
...
*** Quanto à estrutura pública adequada de comunicação paciente-médico
	...
	| V019                                                |   n |     f |
	|-----------------------------------------------------+-----+-------|
	| Ruim, não sabem LIBRAS e sou obrigado a oralizar    |  37 |  0.22 |
	| Ruim, não sabem LIBRAS e preciso escrever português |  25 | 0.149 |
	| Ruim, não consigo comunicação                       |  43 | 0.256 |
	| Boa, tem intérprete que me ajuda                    |  20 | 0.119 |
	| Boa, porque familiar me ajuda                       |  38 | 0.226 |
	| Boa, o profissional de saúde sabe LIBRAS            |   4 | 0.024 |
	| nil                                                 |   1 | 0.006 |
	| Total de respostas válidas                          | 168 |     1 |
	...
	|--------------------------------------------+-----+-------|
	| Sim                                        |  29 | 0.173 |
	| Não, porque eu esqueço de pedir            |  24 | 0.143 |
	| Não, porque não tem intérprete livre nunca |  41 | 0.244 |
	| nil                                        |  74 |  0.44 |
	| Total de respostas válidas                 | 168 |     1 |
	...
	| Ruim, não consigo comunicação                      | 11 |   0.2 |
...
*** Quanto à estrutura privada adequada de comunicação paciente-médico
	...
	| V022                                                |   n |     f |
	|-----------------------------------------------------+-----+-------|
	| Ruim, não sabem LIBRAS e sou obrigado a oralizar    |  39 | 0.219 |
	| Ruim, não sabem LIBRAS e preciso escrever português |  27 | 0.152 |
	| Ruim, não consigo comunicação                       |  57 |  0.32 |
	| Boa, tem intérprete que me ajuda                    |  17 | 0.096 |
	| Boa, porque familiar me ajuda                       |  33 | 0.185 |
...
*** Escolaridade vs Estado de Saúde
	...
	p-valor do teste qui-quadrado para independência entre as variáveis: src_R[:session :results replace ]{ round( ca.pvalue, 6) } {{{results(=0.742724=)}}}.
...
*** Religião vs Estado de Saúde
	...
	p-valor do teste qui-quadrado para independência entre as variáveis: src_R[:session :results replace ]{ round( ca.pvalue, 6) } {{{results(=0.125609=)}}}.
...
*** Trabalho vs Estado de Saúde
	...
	p-valor do teste qui-quadrado para independência entre as variáveis: src_R[:session :results replace ]{ round( ca.pvalue, 6) } {{{results(=0.0476=)}}}.
...
*** Cor/Raça vs Estado de Saúde
	...
	p-valor do teste qui-quadrado para independência entre as variáveis: src_R[:session :results replace ]{ round( ca.pvalue, 6) } {{{results(=0.397374=)}}}.
...
*** Cidade vs Estado de Saúde
	...
	p-valor do teste qui-quadrado para independência entre as variáveis: src_R[:session :results replace ]{ round( ca.pvalue, 6) } {{{results(=0.488594=)}}}.
...
*** Diabetes vs Estado de Saúde
	...
	p-valor do teste qui-quadrado para independência entre as variáveis: src_R[:session :results replace ]{ round( ca.pvalue, 6) } {{{results(=0.103977=)}}}.
...
*** Hipertensão vs Estado de Saúde
	...
	p-valor do teste qui-quadrado para independência entre as variáveis: src_R[:session :results replace ]{ round( ca.pvalue, 6) } {{{results(=0.00047=)}}}.
...
*** Tabagismo vs Estado de Saúde
	...
	p-valor do teste qui-quadrado para independência entre as variáveis: src_R[:session :results replace ]{ round( ca.pvalue, 6) } {{{results(=0.735779=)}}}.
...
*** Faixa etária vs Estado de Saúde
	...
	p-valor do teste qui-quadrado para independência entre as variáveis: src_R[:session :results replace ]{ round( ca.pvalue, 6) } {{{results(=0.725998=)}}}.
