...
** Tratamento dos dados
   ...
   ## validação da lógica...
   ## dat %>% filter( VALIDA == TRUE ) %>% View
   ## dat %>% filter( is.na(V002) ) %>% View
   ## dat %>% select( V002, V003, V004_idade, V010_rmc, V010, V019, V022, VALIDA) %>% View
   ## dat %>% select( V002, V003, V004_idade, V010_rmc, V010, VALIDA) %>% filter( is.na(VALIDA) ) %>% View

   ## zap 19.10.2021 validação da logica

   ## dat %>% filter( !is.na(V002) & V003 == 'Sim' & ( V004_idade >= 18 | is.na(V004_idade) ) & ( V010_rmc == TRUE | is.na(V010) )  ) %>%
   ## dat %>% filter( !is.na(V002) & ( V003 != 'Sim' | is.na(V003) ) & ( V004_idade >= 18                     ) & ( V010_rmc == TRUE               )  ) %>%
   ## dat %>% filter( !is.na(V002) & ( V003 != 'Sim' | is.na(V003) ) & is.na(V004_idade) ) %>% 
   ## dat %>% filter( is.na(V002)  & V003 == 'Sim' & ( V004_idade >= 18 | is.na(V004_idade) ) & ( V010_rmc == TRUE | is.na(V010) ) ) %>%
   ## dat %>% filter( is.na(V002)  & ( V003 != 'Sim' | is.na(V003) ) & ( V004_idade >= 18 ) & ( V010_rmc == TRUE ) ) %>%
   ##   select( V002, V003, V004, V004_idade, V010, VALIDA) %>%
   ##   View
...
** Perfil dos participantes
   ...
   - src_R[:session :results replace ]{ 100*sum( v006[ c(4,5), 'p'] ) } {{{results(=40.3=)}}} % com ensino superior ou pós-graduação
   - src_R[:session :results replace ]{ 100*sum( v015[ c(1,2), 'p'] ) } {{{results(=73.2=)}}} % avaliam seu estado de saúde como bom ou muito bom
...
*** Quanto à escolaridade
	...
	| Médio (ou 2º grau)                                  | 104 | 0.456 |
	| Superior (ou 3º grau)                               |  71 | 0.311 |
	| Pós-graduação (especialização, mestrado, doutorado) |  21 | 0.092 |
	| nil                                                 |   1 | 0.004 |
	| Total de respostas válidas                          | 228 |     1 |
...
*** Quanto à situação de emprego
	...
	| V007                                     |   n |     f |
	|------------------------------------------+-----+-------|
	| Sim, trabalho                            | 145 | 0.636 |
	| Não, estou desempregado                  |  52 | 0.228 |
	| Não, porque sou aposentado               |  17 | 0.075 |
	| Não trabalho e recebo benefício BPC/INSS |  14 | 0.061 |
	| Total de respostas válidas               | 228 |     1 |
...
**** segmentada por nível de escolaridade
	...
	| Médio (ou 2º grau)                                  |           28 |        71 |            28.28 |
	| Superior (ou 3º grau)                               |           15 |        49 |            23.44 |
	| Pós-graduação (especialização, mestrado, doutorado) |            4 |        16 |               20 |
...
*** Quanto à estrutura pública adequada de comunicação paciente-médico
	...
	| V019                                                |   n |     f |
	|-----------------------------------------------------+-----+-------|
	| Ruim, não sabem LIBRAS e sou obrigado a oralizar    |  37 | 0.218 |
	| Ruim, não sabem LIBRAS e preciso escrever português |  26 | 0.153 |
	| Ruim, não consigo comunicação                       |  42 | 0.247 |
	| Boa, tem intérprete que me ajuda                    |  20 | 0.118 |
	| Boa, porque familiar me ajuda                       |  38 | 0.224 |
	| Boa, o profissional de saúde sabe LIBRAS            |   4 | 0.024 |
	| nil                                                 |   3 | 0.018 |
	| Total de respostas válidas                          | 170 |     1 |
	...
	|--------------------------------------------+-----+-------|
	| Sim                                        |  30 | 0.176 |
	| Não, porque eu esqueço de pedir            |  24 | 0.141 |
	| Não, porque não tem intérprete livre nunca |  39 | 0.229 |
	| nil                                        |  77 | 0.453 |
	| Total de respostas válidas                 | 170 |     1 |
	...
	| V025                                               |  n |     f |
	|----------------------------------------------------+----+-------|
	| Ruim, não sabe LIBRAS e sou obrigado a oralizar    | 15 | 0.278 |
	| Ruim, não sabe LIBRAS e preciso escrever português |  7 |  0.13 |
	| Ruim, não consigo comunicação                      | 10 | 0.185 |
	| Boa, tem intérprete que me ajuda                   |  2 | 0.037 |
	| Boa, porque familiar me ajuda                      | 13 | 0.241 |
	| Boa, o profissional de saúde sabe LIBRAS           |  4 | 0.074 |
	| nil                                                |  3 | 0.056 |
	| Total de respostas válidas                         | 54 |     1 |
...
*** Quanto à estrutura privada adequada de comunicação paciente-médico
	...
	| Boa, tem intérprete que me ajuda                    |  16 |  0.09 |
