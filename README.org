...
** Perfil dos participantes
   ...
   Até o momento src_R[:session :results replace ]{ nrow(dat)} {{{results(=92=)}}}  pessoas participaram da pesquisa, provenientes das seguintes cidades
   ...
   Dos quais até o momento src_R[:session :results replace ]{ nrow(dat %>% filter( !is.na(VALIDA)))} {{{results(=80=)}}}  participantes pertencem ao público alvo da pesquisa, provenientes das seguintes cidades
   ...
   - idade média de src_R[:session :results replace ]{ round( v004,1) } {{{results(=39.4=)}}} anos
   - src_R[:session :results replace ]{ round( v005[1,3]*100,1) } {{{results(=56.2=)}}} % do sexo src_R[:session :results replace ]{ v005[1,1] } {{{results(=Feminino=)}}}
   - src_R[:session :results replace ]{ 100*sum( v006[ c(4,5), 'p'] ) } {{{results(=60=)}}} % com ensino superior ou pós-graduação
